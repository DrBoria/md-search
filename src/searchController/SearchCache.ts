...
export class SearchCache {
  ...
  private resultMatchesQuery(
    ...
    // Для оптимизации просто проверяем, есть ли совпадения
    // В реальном сценарии нужно проверить каждое совпадение на соответствие запросу
    return result.matches.some(match => {
      const matchText = result.source!.substring(match.start, match.end);
      
      if (!matchCase) {
        return matchText.toLowerCase().includes(query.toLowerCase());
      }
      
      if (wholeWord) {
        const regex = new RegExp(`\\b${escapeRegExp(query)}\\b`, matchCase ? "" : "i");
