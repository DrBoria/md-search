...
const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
    ({ className, variant, size, asChild = false, active, ...props }, ref) => {
        ...
        return (
            <Comp
                className={cn(buttonVariants({ variant, size, className }), active && "bg-[var(--vscode-inputOption-activeBackground)] text-[var(--vscode-inputOption-activeForeground)] border-[var(--vscode-inputOption-activeBorder)]")}
                ref={ref}
                {...props}
            />
        )
